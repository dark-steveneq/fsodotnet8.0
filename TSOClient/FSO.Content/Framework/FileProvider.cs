...
namespace FSO.Content.Framework
{
    ...
    public class FileProvider<T> : IContentProvider<T>
    {
        ...
        public bool UseTS1;
        public bool FAR3IDs;
        internal static int PoolID = CacheControler.NewPool("File Provider");
        ...
        ~FileProvider() => CacheControler.FreePool(PoolID);
        ...
        public T Get(string name)
        {
            ...
            if (CacheControler.Cached(PoolID, name))
                return CacheControler.Get<T>(PoolID, name);
            else
            {
                T result = ThrowawayGet(name);
                if (result == null)
                    return default(T);
                CacheControler.Cache(PoolID, name, result);
                return result;
            }
        }
